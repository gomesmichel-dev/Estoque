...
void cadastrar_produto(Fl_Widget*, void*){
    try {
        ...
        if (marca_id == -1) {
            std::cerr << "Erro ao inserir ou obter marca." << std::endl;
            return;
        }

        int tipo_id = inserirOuObterTipo(tipo);
        if (tipo_id == -1) {
            std::cerr << "Erro ao inserir ou obter tipo." << std::endl;
            return;
        }
        ...
        if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) != SQLITE_OK) {
            std::cerr << "Erro ao preparar a declaração SQL: " << sqlite3_errmsg(db) << std::endl;
            return;
        }
